...
class CDCConsumer:
    def __init__(self, state_file="cdc_state.txt", poll_interval=5, max_retries=5, initial_backoff=1):
        self.state_file = Path(state_file)
        self.poll_interval = poll_interval
        self.max_retries = max_retries
        self.initial_backoff = initial_backoff
        self.last_processed_id = self._load_last_processed_id()
        logger.info(f"CDC Consumer initialized. Last processed ChangeLog ID: {self.last_processed_id}")
    ...
    def _load_last_processed_id(self):
        ...
        return 0 # Start from the beginning if file doesn't exist or is empty/corrupt
    ...
    async def _execute_with_retry(self, func, *args, **kwargs):
        ...
        while retries < self.max_retries:
            try:
                ...
                logger.warning(f"Attempt {retries + 1}/{self.max_retries} failed for {func.__name__}: {e}. Retrying in {delay} seconds...")
                retries += 1
    ...
    async def _sync_agent_to_milvus(self, agent_id, payload, operation_type):
        logger.info(f"Syncing agent {agent_id} to Milvus (Operation: {operation_type})...")
    ...
    async def _sync_agent_to_neo4j(self, agent_id, payload, operation_type):
        logger.info(f"Syncing agent {agent_id} to Neo4j (Operation: {operation_type})...")
            ...
            if neo4j_driver:
                try:
                    ...
                    if operation_type == "INSERT" or operation_type == "UPDATE":
                        ...
                        await asyncio.to_thread(neo4j_driver.session().run, query, agent_id=agent_id, name=name, role=role, status=status)
                        logger.info(f"Agent {agent_id} upserted to Neo4j.")
                    elif operation_type == "DELETE":
                        query = "MATCH (a:Agent {agent_id: $agent_id}) DETACH DELETE a"
                        await asyncio.to_thread(neo4j_driver.session().run, query, agent_id=agent_id)
                        logger.info(f"Agent {agent_id} deleted from Neo4j.")
                except Exception as e:
                    logger.error(f"Error syncing agent {agent_id} to Neo4j: {e}\n{traceback.format_exc()}")
        ...
        async with neo4j_connection_context() as neo4j_driver:
            if neo4j_driver:
                try:
                    ...
                    logger.error(f"Error deleting agent {agent_id} from Neo4j: {e}\n{traceback.format_exc()}")

    async def _sync_document_to_milvus(self, document_id, payload, operation_type):
        logger.info(f"Syncing document {document_id} to Milvus (Operation: {operation_type})...")
            ...
            if milvus_client:
                try:
                    if operation_type == "INSERT" or operation_type == "UPDATE":
                        ...
                        logger.warning(f"Document {document_id} change detected. Milvus sync for full documents is a placeholder. Chunks should be synced separately.")
                    elif operation_type == "DELETE":
                        ...
                        logger.info(f"Document {document_id} deleted. Placeholder for deleting associated chunks from Milvus.")
                except Exception as e:
                    logger.error(f"Error syncing document {document_id} to Milvus: {e}\n{traceback.format_exc()}")
        ...
        async with milvus_connection_context() as milvus_client:
            if milvus_client:
                try:
                    ...
                    logger.info(f"Placeholder: Deleting all chunks for document {document_id} from Milvus.")
                except Exception as e:
    ...
    async def _sync_document_to_neo4j(self, document_id, payload, operation_type):
        logger.info(f"Syncing document {document_id} to Neo4j (Operation: {operation_type})...")
        async with neo4j_connection_context() as neo4j_driver:
            if neo4j_driver:
                try:
                    ...
                    if operation_type == "INSERT" or operation_type == "UPDATE":
                        ...
                        await asyncio.to_thread(neo4j_driver.session().run, query, document_id=document_id, title=title, source_url=source_url)
                        logger.info(f"Document {document_id} upserted to Neo4j.")
                    elif operation_type == "DELETE":
                        query = "MATCH (d:Document {document_id: $document_id}) DETACH DELETE d"
                        await asyncio.to_thread(neo4j_driver.session().run, query, document_id=document_id)
                        logger.info(f"Document {document_id} deleted from Neo4j.")
                except Exception as e:
                    logger.error(f"Error syncing document {document_id} to Neo4j: {e}\n{traceback.format_exc()}")
        ...
        async with neo4j_connection_context() as neo4j_driver:
            if neo4j_driver:
                try:
                    ...
                    logger.error(f"Error deleting document {document_id} from Neo4j: {e}\n{traceback.format_exc()}")

    async def _sync_chunk_to_milvus(self, chunk_id, payload, operation_type):
        logger.info(f"Syncing chunk {chunk_id} to Milvus (Operation: {operation_type})...")
        async with milvus_connection_context() as milvus_client:
            if milvus_client:
                try:
                    if operation_type == "INSERT" or operation_type == "UPDATE":
                        ...
                        if text_content:
                            embedding = await get_embedding(text_content) # Assuming get_embedding is an async function
                            if embedding:
                                data = {
                                    "id": str(chunk_id),
                                    "embedding": embedding,
                            ...
                            logger.warning(f"No text content found in payload for chunk {chunk_id} to sync to Milvus.")
                    elif operation_type == "DELETE":
                    ...
                    logger.error(f"Error syncing chunk {chunk_id} to Milvus: {e}\n{traceback.format_exc()}")
        ...
        async with milvus_connection_context() as milvus_client:
            if milvus_client:
                try:
                    ...
                    logger.error(f"Error deleting chunk {chunk_id} from Milvus: {e}\n{traceback.format_exc()}")

    async def _sync_chunk_to_neo4j(self, chunk_id, payload, operation_type):
        logger.info(f"Syncing chunk {chunk_id} to Neo4j (Operation: {operation_type})...")
        async with neo4j_connection_context() as neo4j_driver:
            if neo4j_driver:
                try:
                    ...
                    if operation_type == "INSERT" or operation_type == "UPDATE":
                        ...
                        await asyncio.to_thread(neo4j_driver.session().run, query, chunk_id=chunk_id, text=text, document_id=document_id, model_name=payload.get("ModelName"), model_version=payload.get("ModelVersion"))
                        logger.info(f"Chunk {chunk_id} upserted to Neo4j.")
                    elif operation_type == "DELETE":
                        query = "MATCH (c:Chunk {chunk_id: $chunk_id}) DETACH DELETE c"
                        await asyncio.to_thread(neo4j_driver.session().run, query, chunk_id=chunk_id)
                        logger.info(f"Chunk {chunk_id} deleted from Neo4j.")
                except Exception as e:
                    logger.error(f"Error syncing chunk {chunk_id} to Neo4j: {e}\n{traceback.format_exc()}")
        ...
        async with neo4j_connection_context() as neo4j_driver:
            if neo4j_driver:
                try:
                    ...
                    logger.error(f"Error deleting chunk {chunk_id} from Neo4j: {e}\n{traceback.format_exc()}")

    async def _process_change(self, change):
        change_id, source_table, source_id, change_type, payload_json = change
        logger.info(f"Processing change: ID={change_id}, Table={source_table}, "
                    f"SourceID={source_id}, Type={change_type}")
        try:
            payload = json.loads(payload_json)
        except json.JSONDecodeError:
            logger.error(f"Failed to decode JSON payload for ChangeLog ID {change_id}. Skipping.")
            return

        sync_tasks = []
        if source_table == "Agents":
            if change_type == "INSERT" or change_type == "UPDATE":
                sync_tasks.append(self._execute_with_retry(self._sync_agent_to_neo4j, source_id, payload, change_type))
            elif change_type == "DELETE":
                sync_tasks.append(self._execute_with_retry(self._delete_agent_from_neo4j, source_id))
        elif source_table == "Documents":
            if change_type == "INSERT" or change_type == "UPDATE":
                sync_tasks.append(self._execute_with_retry(self._sync_document_to_milvus, source_id, payload, change_type))
                sync_tasks.append(self._execute_with_retry(self._sync_document_to_neo4j, source_id, payload, change_type))
            elif change_type == "DELETE":
                sync_tasks.append(self._execute_with_retry(self._delete_document_from_milvus, source_id))
                sync_tasks.append(self._execute_with_retry(self._delete_document_from_neo4j, source_id))
        elif source_table == "Chunks":
            if change_type == "INSERT" or change_type == "UPDATE":
                sync_tasks.append(self._execute_with_retry(self._sync_chunk_to_milvus, source_id, payload, change_type))
                sync_tasks.append(self._execute_with_retry(self._sync_chunk_to_neo4j, source_id, payload, change_type))
            elif change_type == "DELETE":
                sync_tasks.append(self._execute_with_retry(self._delete_chunk_from_milvus, source_id))
                sync_tasks.append(self._execute_with_retry(self._delete_chunk_from_neo4j, source_id))
        ...
        while True:
            try:
                ...
                async with sql_server_connection_context(sql_server_conn) as (conn, cursor):
                    if not conn:
                        logger.error("SQL Server connection is not available. Retrying...")
                ...
                logger.error(f"Error in CDC consumer main loop: {e}\n{traceback.format_exc()}")
