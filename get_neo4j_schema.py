...
async def get_neo4j_schema():
    ...
    try:
        ...
        if driver is None:
            schema_info = {"error": "Failed to get Neo4j driver from db_connectors.", "neo4j_uri": NEO4J_URI}
        ...
        async with driver.session() as session:
            ...
            for label in node_labels:
                ...
                try:
                    sample_node_result = await session.run(f"MATCH (n:{label}) RETURN properties(n) LIMIT 1")
            ...
            for rel_type in rel_types:
                ...
                try:
                    sample_rel_result = await session.run(f"MATCH ()-[r:{rel_type}]->() RETURN properties(r) LIMIT 1")
                    sample_properties = await sample_rel_result.single()
                    if sample_properties:
                        schema_info["relationships"][rel_type] = list(sample_properties[0].keys())
                except Exception as e:
                    pass
        ...
        schema_info = {"error": f"An unexpected error occurred: {e}", "neo4j_uri": NEO4J_URI}
        print(json.dumps(schema_info, indent=2)) # Print error to stdout
    finally:
        if driver:
            await driver.close()
