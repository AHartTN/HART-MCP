...
def main():
    parser = argparse.ArgumentParser(description="Hartonomous CLI client for MCP server.")
    parser.add_argument("prompt", type=str, help="The mission prompt for the agent.")
    parser.add_argument("--host", type=str, default="http://127.0.0.1", help="The MCP server host.")
    parser.add_argument("--port", type=int, default=8000, help="The MCP server port.")
    parser.add_argument("--agent_id", type=int, default=1, help="The agent ID to use for the mission.")
                    ...
                    if decoded_line.startswith("data: "):
                        try:
                            ...
                            if "status" in message and message["status"] == "completed":
                                print(f"\033[92m{json.dumps(message, indent=2)}\033[0m") # Green for completion
                            elif "error" in message:
                                print(f"\033[91m{json.dumps(message, indent=2)}\033[0m") # Red for errors
                            else:
