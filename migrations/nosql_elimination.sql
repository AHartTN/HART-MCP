...
CREATE TABLE VectorEmbeddings (
    ...
    Embedding_OpenAI VECTOR(1536) NOT NULL,          -- OpenAI embeddings
    Embedding_SentenceTransformer VECTOR(384),       -- Sentence transformer
    Embedding_Custom VECTOR(768),                    -- Custom model embeddings
    ...
    SubCategory JSON, -- Hierarchical categories as JSON array
...
(
    ...
    TextResults AS (
        SELECT 
            EmbeddingID,
            CONTAINS(ChunkText, @QueryText) as TextMatch,
            -- Calculate text similarity score
            (LEN(@QueryText) - LEN(REPLACE(LOWER(ChunkText), LOWER(@QueryText), ''))) / LEN(@QueryText) as TextScore
        FROM VectorEmbeddings
        WHERE CONTAINS(ChunkText, @QueryText)
    ),
    CombinedResults AS (
        SELECT TOP (@TopK)
            v.EmbeddingID,
...
CREATE TABLE GraphNodes (
    ...
    NodeType NVARCHAR(100) NOT NULL, -- Agent, Document, Concept, Tool, etc.
    ...
    WITH GraphCTE AS (
        ...
        WHERE g.Depth < @MaxDepth
          AND (@RelationshipTypes IS NULL OR 
               ...
               r.RelationshipType IN (SELECT value FROM OPENJSON(@RelationshipTypes)))
          AND CHARINDEX(CAST(n.NodeID as NVARCHAR(MAX)), g.Path) = 0 -- Prevent cycles
    )
...
(
    ...
    WHERE NodeEmbedding IS NOT NULL
      AND (@NodeTypes IS NULL OR 
           NodeType IN (SELECT value FROM OPENJSON(@NodeTypes)))
...
CREATE OR ALTER PROCEDURE sp_UniversalIntelligentSearch
    @Query NVARCHAR(MAX),
    @QueryType NVARCHAR(50) = 'AUTO', -- AUTO, VECTOR, GRAPH, FULLTEXT, HYBRID
    @TopK INT = 10,
...
BEGIN
    ...
    BEGIN
        ...
        SELECT NodeID FROM fn_SemanticGraphSearch(@QueryEmbedding, NULL, 5);
