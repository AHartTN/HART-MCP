...
def test_health_endpoint(client):
    with patch("utils.check_database_health", new_callable=AsyncMock) as mock_check_database_health:
        with patch("utils.get_milvus_client", new_callable=AsyncMock) as mock_get_milvus_client:
            with patch("utils.get_neo4j_driver", new_callable=AsyncMock) as mock_get_neo4j_driver:
                with patch("utils.get_sql_server_connection", new_callable=AsyncMock) as mock_get_sql_server_connection:
