...
async def test_search_milvus_async_success(monkeypatch):
    ...
    monkeypatch.setattr("utils.milvus_connection_context", mock_milvus_context)
...
async def test_search_neo4j_async_success(monkeypatch):
    ...
    monkeypatch.setattr("utils.neo4j_connection_context", mock_neo4j_context)
...
async def test_search_sql_server_async_success(monkeypatch):
    ...
    monkeypatch.setattr("utils.sql_connection_context", mock_sql_context)
...
async def test_rag_orchestrator_generate_response_success(
    ...
    monkeypatch.setattr(
        "services.database_search.search_milvus_async",
        AsyncMock(return_value=[{"text": "milvus context"}]),
    )
    monkeypatch.setattr(
        "services.database_search.search_neo4j_async",
        AsyncMock(return_value=["neo4j context"]),
    )
    monkeypatch.setattr(
        "services.database_search.search_sql_server_async",
