...
class FunctionalTestSuite:
        ...
        try:
            ...
            try:
                ...
                async with connection_manager as conn:
                    ...
                    row = cursor.fetchone()
                    if row and row[0] == 1:
                        results["sql_server"] = {"connected": True, "query_test": "passed"}
                        logger.info("✅ SQL Server functional test passed")
                    else:
                        results["sql_server"] = {"connected": True, "query_test": "failed"}
            ...
            try:
                ...
                if client:
                    ...
                    logger.info(f"✅ Milvus functional test passed - {len(collections)} collections")
                else:
                    results["milvus"] = {"connected": False, "error": "Failed to create Milvus client"}
            ...
            return {
                ...
                "functional_operations_passed": sum(1 for db in results.values() if db.get("connected") and db.get("query_test") == "passed")
            }
        ...
        try:
            ...
            try:
                ...
                if "TEST_SUCCESS" in response:
                    results["primary_llm_call"] = {"success": True, "response_length": len(response)}
                    logger.info("✅ Primary LLM call successful")
                else:
                    results["primary_llm_call"] = {"success": False, "response": response[:100]}
            ...
            try:
                ...
                working_models = sum(1 for model in models.values() if model.get("status"))
                results["working_models_count"] = working_models
                logger.info(f"✅ Found {working_models} working models out of {len(models)}")
            ...
            try:
                ...
                if fallback_response and "error" not in fallback_response.lower():
                    results["fallback_test"] = {"success": True, "response_length": len(fallback_response)}
                    logger.info("✅ Fallback functionality working")
                else:
                    results["fallback_test"] = {"success": False, "response": fallback_response[:100]}
                    warnings.append("Fallback functionality may not be working properly")
            ...
            return {
                ...
                "errors": errors,
                "warnings": warnings,
                "functional_tests_passed": sum(1 for test in results.values() if isinstance(test, dict) and test.get("success"))
            }
    ...
    async def test_tool_functionality(self) -> Dict[str, Any]:
        ...
        try:
            from plugins_folder.tools import ToolRegistry, WriteFileTool, ReadFileTool, ListFilesTool
            ...
            try:
                ...
                write_result = write_tool.execute(file_path=test_file, content=test_content)
                
                if write_result.get("status") == "success" and os.path.exists(test_file):
                    ...
                    results["write_tool"] = {"functional": False, "result": write_result}
            ...
            try:
                ...
                if (read_result.get("status") == "success" and 
                    read_result.get("content") == test_content):
            ...
            try:
                ...
                if (list_result.get("status") == "success" and 
                    isinstance(list_result.get("files"), list)):
                    results["list_tool"] = {
                        "functional": True, 
            ...
            try:
                ...
                logger.info(f"✅ Tool registry functional test passed - {len(tool_names)} tools registered")
    ...
    async def test_consciousness_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                from plugins_folder.meta_consciousness import MetaConsciousnessEngine, QuantumThought, ConsciousnessLevel, ThoughtType
                import time
                import uuid
                ...
                results["quantum_thought"] = {
                    ...
                    "consciousness_level_valid": thought.consciousness_level == ConsciousnessLevel.REFLECTIVE
                }
                
                # Test basic methods if they exist
                if hasattr(thought, 'evolve'):
                    evolved = thought.evolve(time_delta=0.1)  # Provide required time_delta parameter
            ...
            for module_name, class_name in consciousness_modules:
                try:
                    module = __import__(f"plugins_folder.{module_name}", fromlist=[class_name])
                    cls = getattr(module, class_name)
                    
                    results[module_name] = {
                        "importable": True,
                        "class_exists": cls is not None,
                        "has_methods": len([m for m in dir(cls) if not m.startswith('_')]) > 0
                    }
    ...
    async def test_api_functionality(self) -> Dict[str, Any]:
        ...
        try:
            ...
            results = {
                ...
                "app_configured": hasattr(server.app, 'title') if hasattr(server, 'app') else False
            }
            ...
            return {
                ...
                "api_results": results,
                "errors": [],
                "warnings": ["Full API testing requires server startup - only tested configuration"],
    ...
    async def test_configuration_validation(self) -> Dict[str, Any]:
        ...
        try:
            ...
            required_configs = [
                'LLM_SOURCE', 'LLM_FALLBACK_ENABLED', 'LLM_FALLBACK_ORDER',
                'GEMINI_MODEL_NAME', 'CLAUDE_MODEL_NAME', 
                'SQL_SERVER_SERVER', 'MILVUS_HOST', 'NEO4J_URI'
            ]
