...
async def run_health_checks():
    ...
    try:
        ...
        for db_name, status in health_status.items():
            ...
            logger.info(f"{status_icon} {db_name.upper()}: {'Healthy' if status else 'Unhealthy'}")
...
async def main():
    ...
    logger.info(f"📊 Setup completed: {success_count}/{len(setup_functions)} databases configured")
